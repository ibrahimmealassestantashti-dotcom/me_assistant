...
def extract_session_metrics_with_ai(service, session_info, api_key, model_name):
    ...
    try:
        ...
        2. نظام الترميز في ورقة الحضور (مهم جداً، اتبعه بدقة):
           - الحرف "م" بخط/لون أزرق أمام اسم الشخص = حاضر (Present).
           - الحرف "غ" بخط/لون أحمر أمام اسم الشخص = غائب (Absent).
           - في أعلى الورقة يوجد مربع صغير يحتوي على الأعداد الإجمالية الجاهزة (عدد الحضور ) مكتوبة يدوياً من معدّ الورقة - اعتمد على هذه الأرقام المكتوبة في المربع الصغير كمصدر أساسي وموثوق للعدد الإجمالي، ولا تكتفِ بعدّ الأسماء يدوياً وقارن النتائج بما قمت بعده واذا لم تتطابق ضع الرقم 100 .
        3. تصنيف كل شخص "حاضر" (م بالأزرق) إلى فئته حسب عمود العمر وعمود الجنس المسجلين بجانب اسمه في نفس الورقة:
           - إذا كان العمر 18 سنة أو أكثر والجنس ذكر → يُحتسب ضمن "الرجال (Men)".
           - إذا كان العمر 18 سنة أو أكثر والجنس أنثى → تُحتسب ضمن "النساء (Women)".
           - إذا كان العمر أقل من 18 والجنس ذكر → يُحتسب ضمن "الأولاد (Boys)".
           - إذا كان العمر أقل من 18 والجنس أنثى → تُحتسب ضمن "الفتيات (Girls)".
           - الأشخاص "الغائبون" (غ بالأحمر) لا يُحتسبون ضمن أي فئة من هذه الفئات إطلاقاً.
        4. عمود ذوي الاحتياجات (PWD): إذا كان الشخص حاضراً (م بالأزرق) وكانت الخانة المقابلة له في عمود "ذوي الاحتياجات" مكتوب فيها "نعم"، يُحتسب هذا الشخص +1 ضمن "ذوي الاحتياجات (PWD)" بالإضافة إلى احتسابه أصلاً ضمن فئته العمرية/الجندرية (رجل/امرأة/ولد/فتاة) - أي أن عدد ذوي الاحتياجات هو عدّ إضافي (تصنيف متقاطع) وليس فئة منفصلة تُطرح من باقي الفئات.
        5. التقرير ملف وورد (Word) أو PDF والبيانات المطلوبة موجودة في النصف الأول من الصفحة الأولى.
        6. عدم وجود رقم أو خانة فارغة تماماً (لا يوجد فيها أي كتابة إطلاقاً) يعني أن القيمة هي صفر (0).
        7. ممنوع منعاً باتاً التخمين أو كتابة أي رقم لست متأكداً منه بشكل قاطع. إذا كانت الخانة تحتوي على كتابة/رقم لكنه غير واضح أو يصعب قراءته بدقة (خط يد غامض، تصوير غير واضح، رقم متداخل أو مشطوب)، اكتب القيمة "غير واضح" حرفياً بدل تخمين أي رقم. لا تفرّق بين احتمالين متقاربين (مثل 6 أو 8) وتختار واحداً عشوائياً - اكتب "غير واضح" في هذه الحالة.
        ...
        - differences: [تقييم لكل بند: اكتب "مطابقة" إذا كانت متطابقة تماماً، أو اكتب وصف الفارق إذا وُجد اختلاف، أو اكتب "⚠️ قيمة غير واضحة" إذا كانت إحدى القيمتين (ورقة الحضور أو التقرير) "غير واضح"]
