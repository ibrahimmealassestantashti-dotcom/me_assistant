...
def extract_session_metrics_with_ai(service, session_info, api_key, model_name):
    ...
    try:
        ...
        2. نظام الترميز في ورقة الحضور (مهم جداً، اتبعه بدقة):
           ...
           - في أعلى الورقة يوجد مربع صغير يحتوي على الأعداد الإجمالية الجاهزة (عدد الحضور وعدد الغياب) مكتوبة يدوياً من معدّ الورقة - اعتمد على هذه الأرقام المكتوبة في المربع الصغير كمصدر أساسي وموثوق للعدد الإجمالي وتوزيع الحضور، ولا تكتفِ بعدّ الأسماء يدوياً إلا إذا كان المربع غير واضح أو غير موجود.
           - "العدد الإجمالي" و"الحضور حسب الفئة (رجال/نساء/أولاد/فتيات/PWD)" المطلوبة أدناه تُقرأ من هذا المربع، وليس من عدّ رموز "م" في القائمة فقط.
        3. التقرير ملف وورد (Word) أو PDF والبيانات المطلوبة موجودة في النصف الأول من الصفحة الأولى.
        4. عدم وجود رقم أو خانة فارغة يعني تماماً أن القيمة هي صفر (0).
