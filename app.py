[source: 1]import streamlit as st
import json
import os
import time
import io
...
def analyze_session_files_with_ai(service, session_info, api_key):
    # استخدام الذاكرة المؤقتة لمنع إرسال طلبات متكررة لنفس الجلسة
    ...
    try:
        ...
        # استخدام نموذج 1.5 الفعال والمستقر مجاناً
                    ...
                    if current_view == "attachments":
                        ...
                        if not GEMINI_API_KEY:
                            ...
                            for i, sess in enumerate(sessions_data):
                                ...
                                # مهلة زمنية قصيرة بين الطلبات لتجنب الضغط على الخادم
                                if i > 0:
                                    time.sleep(4)
                                ...
                                with st.chat_message("assistant"):
                                    with st.spinner("جاري تحليل ومراجعة محتوى الملفات والإجابة..."):
                                        ...
                                        try:
                                            ...
                                            # تصحيح استدعاء نموذج المحادثة بالطريقة الصحيحة
                                            response = client.models.generate_content(
                                                model="gemini-1.5-flash",
                                                contents=ai_prompt,
                                            )
